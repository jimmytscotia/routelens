...
def test_http_check_from_response_classifies_2xx_as_healthy():
    result = http_check_from_response("https://web.internal.example/", 200, 982, error=None)
...
def test_http_check_from_response_classifies_error_as_critical():
    result = http_check_from_response("https://web.internal.example/", None, 0, error="timeout")
...
def test_tls_expiry_status_warns_inside_30_days():
    result = tls_expiry_status(days_remaining=12, issuer="Lets Encrypt", sans=["web.internal.example"])
...
def test_tls_expiry_status_healthy_with_long_validity():
    result = tls_expiry_status(days_remaining=88, issuer="Lets Encrypt", sans=["web.internal.example"])
